...
{
    if(self.baseDelegate && [self.baseDelegate respondsToSelector:@selector(tabBarController:shouldSelectViewController:index:)]){
        WWBaseNavigationController *navigationController = nil;
        if([viewController isKindOfClass:NSClassFromString(@"WWBaseNavigationController")]){
            navigationController = (WWBaseNavigationController *)viewController;
            NSUInteger index = [self.rootControllers indexOfObject:NSStringFromClass(navigationController.viewControllers[0].class)];
            return [self.baseDelegate tabBarController:self shouldSelectViewController:viewController index:index];
        }
    }
    return YES;
}
...
{
    if(self.baseDelegate && [self.baseDelegate respondsToSelector:@selector(tabBarController:didSelectViewController:index:)]){
        WWBaseNavigationController *navigationController = nil;
        if([viewController isKindOfClass:NSClassFromString(@"WWBaseNavigationController")]){
            navigationController = (WWBaseNavigationController *)viewController;
            NSUInteger index = [self.rootControllers indexOfObject:NSStringFromClass(navigationController.viewControllers[0].class)];
            return [self.baseDelegate tabBarController:self didSelectViewController:viewController index:index];
        }
    }
}
